...
public class GameUI : Singleton<GameUI>
{
    ...
    private Dictionary<UIElement, int> SliderElements;
    private UIElement currentSliderElements;
    ...
    protected override void Awake()
    {
        ...
        SliderElements = new Dictionary<UIElement, int>();
    ...
    public void AddSliderElements(UIElement type, int index)
    {
        if (!SliderElements.ContainsKey(type))
        {
            SliderElements.Add(type, index);
        }

        if (currentSliderElements == null)
        {
            currentSliderElements = type;
            return ;
        }

        int a = SliderElements[type]; 
        int b = SliderElements[currentSliderElements]; 

        int min = Math.Min(a, b);
        int max = Math.Max(a, b);

        List<KeyValuePair<UIElement, int>> filteredElements = SliderElements
           .Where(element => element.Value >= min && element.Value <= max)
           .ToList();

        List<KeyValuePair<UIElement, int>> SortedSlider;
        if (a - b < 0) 
        {
            SortedSlider = filteredElements.OrderBy(element => element.Value).ToList();
        }
        else 
        {
            SortedSlider = filteredElements.OrderByDescending(element => element.Value).ToList();
        }

        int i = 2;
        foreach (var element in SortedSlider)
        {
            //Debug.Log($"Type: {element.Key}, Value: {element.Value}, Time: {element.Value - b}");
            element.Key.gameObject.transform.SetSiblingIndex(transform.childCount - i);
            element.Key.StartInCorner(element.Value - b, SortedSlider.Count - 1);
            i++;
        }
    ...
    public void EnableAllSliderElement()
    {
        Get<UIShop>().Show();
        Get<UITask>().Show();
        Get<UIChallenge>().Show();
        Get<UIHome>().Show();
        Get<UILoadingInGame>().Show();
    }
#if UNITY_EDITOR
